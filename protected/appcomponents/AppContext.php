...
class AppContext extends \CApplicationComponent {
     ...
     * Get current planet
     *
     * @return ZPlanet return null on not found
     */
    public function getPlanet(){
        ...
        if (!$this->_planet) {
            ...
            if ($planet_id) {
                $planet = ZPlanet::model()->with('planetData', 'tasks')->findByPk($planet_id);
                if ($planet) {
                    $planet->owner = $this->getPlayer();
     ...
     * @throws CException
     */
    public function switchPlanet(ZPlanet $planet){
